...
def debug(args,io):
    ...
    if (len(train_dataset) < 100):
        ...
        drop_last = True
    train_loader = DataLoader(train_dataset, num_workers=8, batch_size=args.batch_size, shuffle=True, drop_last=drop_last) #8的batchsize 16类one-hot
...
def train(args, io):
    ...
    net = torch.load(model_path) #model load 了 pretrain的
    model.load_state_dict(net, strict=False)
    
    print("Model Loaded!!!")

    if args.use_sgd: #默认是True
        print("Use SGD")
        opt = optim.SGD(model.parameters(), lr=args.lr *100, momentum=args.momentum, weight_decay=1e-4)
    else:
        print("Use Adam")
    ...
    wandb.init(project="CrossPoint", name=args.exp_name) #输入是dgcnn_partseg
    wandb.watch(model)
    ...
    for epoch in range(args.epochs):
        ...
        wandb_log["Train Mean IOU"] = np.mean(train_ious)
        wandb_log["Train Loss"] = train_loss*1.0/count
        io.cprint(outstr)

        ####################
        # Test 每个epoch都Test与输出
        ####################
        test_loss = 0.0
        count = 0.0
        model.eval()
        ...
        wandb_log["Test Mean IOU"] = np.mean(test_ious)
        wandb_log["Test Loss"] = test_loss*1.0/count
        io.cprint(outstr)
        if np.mean(test_ious) >= best_test_iou: #记录test IoU最好的model到output里
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=200, metavar='N',
                        help='number of episode to train ') #输入是300 太大了吧
    ...
    parser.add_argument('--eval', type=bool,  default=False,
                        help='evaluate the model') #默认就是train，不过有pretrain model load
    ...
    parser.add_argument('--model_path', type=str, default='', metavar='N',
                        help='Pretrained model path') #这个是给test load的
    parser.add_argument('--pretrained_path', type=str, default='', metavar='N',
                    help='Pretrained model path') #这个输入是 ./models/dgcnn_partseg_best.pth 这个是我从网上下下来的
